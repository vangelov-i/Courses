namespace Assertions_Homework
{
    ...
    internal class Assertions
    {
        ...
        {
            Debug.Assert(arr != null && arr.Length > 0, "The array is null or empty.");

            for (int index = 0; index < arr.Length - 1; index++)
            {
                int minElementIndex = FindMinElementIndex(arr, index, arr.Length - 1);
                Debug.Assert(arr[minElementIndex].CompareTo(arr[index]) <= 0, "Finding minElementIndex failed.");
        ...
        {
            Debug.Assert(startIndex >= 0 && endIndex < arr.Length, "endIndex is outside the boundaries of the arrray.");

            int minElementIndex = startIndex;
            for (int currentIndex = startIndex + 1; currentIndex <= endIndex; currentIndex++)
            {
                if (arr[currentIndex].CompareTo(arr[minElementIndex]) < 0)
                {
